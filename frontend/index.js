    ...
    analyzeBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!recommendationsResponse.ok) {
                ...
                throw new Error(`Ошибка получения рекомендаций: ${recommendationsResponse.status} - ${errorText}`);
            }
            ...
            if (Object.keys(hardSkills).length > 0) {
                const hardSkillsHTML = `
                    <div class="hard-skills-container">
                        ${Object.entries(hardSkills).map(([skill, level]) => `
            ...
            recommendationsData.roles.forEach((role, index) => {
                const skillsToImprove = Object.entries(role.skills)
                    .filter(([skill, req]) => req > (uploadData.candidate.hard_skills[skill] || 0))
                    .sort((a, b) => {
                ...
                const strongSkills = Object.entries(uploadData.candidate.hard_skills)
                    .filter(([skill, level]) => role.skills[skill] && level > role.skills[skill])
                    .map(([skill, level]) => `<li>${skill} (уровень ${level} - ${getLevelLabel(level)}, требуется ${role.skills[skill]})</li>`)
                    .join('');

                const additionalSkills = Object.entries(uploadData.candidate.hard_skills)
                    .filter(([skill, level]) => !role.skills[skill] || role.skills[skill] === 0)
                    .map(([skill, level]) => `<li>${skill} (уровень ${level} - ${getLevelLabel(level)})</li>`)
                    .join('');

                const roleHTML = `
                    <div class="role-block ${index === 0 ? 'highlighted-role' : ''}">
                        <div class="role-header">
                            <h4>${index === 0 ? 'Наиболее подходящая профессия: ' : 'Другие профессии: '}${role.name}</h4>
                            <span class="match-percent">${role.match_percent}% совпадение</span>
                        </div>
                            ...
                            <table class="skills-table">
                                ...
                                <tbody>
                                    ${skillsToImprove.length > 0 ?
                                        skillsToImprove.map(([skill, req]) => {
                                            const userLevel = uploadData.candidate.hard_skills[skill] || 0;
                                            const gap = req > 0 ? Math.max(req - userLevel, 0) : 0;
            ...
            if (recommendationsData.soft_skills?.length > 0) {
                const softSkillsHTML = `
                    <div class="soft-skills">
                        <h4>Soft skills:</h4>
                        <ul>
                            ${recommendationsData.soft_skills.map(skill => `<li>${skill}</li>`).join('')}
                        </ul>
                    </div>
                `;
